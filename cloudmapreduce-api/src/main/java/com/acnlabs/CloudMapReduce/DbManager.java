...
public class DbManager {
	...
	static final String dbManagerDomain = "CloudMapReduce"; // creating domains with this name --d3
	...
	private AmazonSimpleDB service; // the main DB interface!! --d3
	...
	public PerformanceTracker perf = new PerformanceTracker(); // timer to track performace using hashmap --d3
	private Logger  logger = Logger.getLogger("com.acnlabs.CloudMapReduce.DbManager"); // cannot find org.apache.log4j in the project though there is a log4j jar file! --d3
    ...
    public DbManager(String accessKeyId, String secretAccessKey, int numMap, int numReduce) {
    	List<String> domainNames = null; // to store domain names --d3
    	Boolean retry = false; // to retry if anything do not follow up --d3
        ...
        workers = new WorkerThreadQueue(Global.numSDBDomain, "DbManager"); //number of threads and names --d3
        ...
        if ( Global.clientID == 0 )  { // only node 0 creates the domain
        	...
        	if ( !domainNames.contains(dbManagerDomain) ) // the domain cloudmapreduce is in list or not --d3
        ...
        numSplits = numMap; // splits equal to number of mappers!--d3
	...
	public void startTask(String jobID, String taskID, String phase) {
		...
		updateStatus(jobID, taskID, phase, "running"); // updating status for a particular id --d3
	}
	 ...
	 * @param minimumTasks the minimum number of tasks needed to continue.  ?? batch!?
	 */
	public boolean isPhaseComplete(String jobID, String phase, int minimumTasks) {
		...
		int all = getPhaseSize(jobID, phase, null); //how many in total --d3
		int running = getPhaseSize(jobID, phase, "running"); // how many running --d3
    ...
    private class CreateDomainRunnable implements Runnable {
    	...
    	public void run() {
			// Check first to see if the domain is already created ?? // we come here only if domain "cloudmr" is not created then why to check again? --d3
			boolean retry;
			int backoff = 1000;
	 ...
	 * Use !exponential backoff! (check this --d3) for now since this is only done once per phase, could evaluate whether spread across domains
	 * 
	 * @param jobID the job ID
	 * @param taskID the task ID
	...
	private int getPhaseSize(String jobID, String phase, String status) {
        ...
        try {
            ...
            if (response.isSetSelectResult()) { // cant find --d3
            	SelectResult  selectResult = response.getSelectResult(); //query the domain --d3
                java.util.List<Item> itemList = selectResult.getItem(); //save the same in list itemList --d3
                /**
                * ?? why iterate? to get size, no option like size?
                my understanding-- domain might have other values which 
                are not of our use so maybe iterate those that are required --d3
                 */
	 ...
	 *  .......       ......
	 *  
	 * Note that we do not require every reducer to update SimpleDB. 
	 * This is assuming either the task writes the result once at the end to the output queue (atomic commit result) 
	 * or the task does not fail during the processing of a reduce key.
	 * Under these assumptions, there is no need to commit the output results, like we did for Map.
	 * 
	 * Most MapReduce jobs meet this requirement. However, it one does not, i.e.,
	 *  if a reduce needs to continuously write output data, there are two solutions:
	 ...
	 */
	// bit confusing do check again --d3
	public String resolveReduce(String jobID, String reduceKey) {
		...
		do {
			retry = false;   // assume success
			try {
				GetAttributesRequest request = new GetAttributesRequest().withItemName(jobID + "_reduce_"+reduceKey).withAttributeName("ClaimedBy"); //item name and attribute name --d3
				GetAttributesResponse response = service.getAttributes(request.withDomainName(dbManagerDomain)); // fired to our domain --d3
		    
				if (response.isSetGetAttributesResult()) { //?? can't find --d3
	...
	public void commitTask(String jobID, String taskID, int id, Global.STAGE stage) {
		...
		ReplaceableAttribute[] attributes = new ReplaceableAttribute[3];
		if ( stage == Global.STAGE.MAP  ) // if map task finished --d3
			attributes[0] = new ReplaceableAttribute("MapCompletedBy", taskID + "_map" + id , true);
		else  // if reducer task finish --d3
...
//-----------?????? used below!!//
    ...
    private class CollectCommittedTaskRunnable implements Runnable {
    	...
    	public void run() {
    		...
    		SelectRequest request = new SelectRequest();
    		do {
    			if (stage == Global.STAGE.MAP) // for mappers commited task --d3
    			...
    			else // for reducers commited task --d3
    			...
    			if ( hasNextToken ) // for limit more than 2500 --d3 
    				request.setNextToken(nextToken);
    			try {
    				SelectResponse response = service.select(request);
    				if (stage == Global.STAGE.MAP )
    					insertCommittedTaskWinners(mapWinners, response); // add to winners list --d3
    				else
	...
	/**
	 * Return a list of Task,mapId or Task,reduceId pair as a result of duplicate resolution to remove
	 * 1) duplicate from eventual consistency (for map only) 2) partial results from failed map/reduce
	 * 
	 * @param jobID the job ID.
	...
	public HashSet<String> getCommittedTask(String jobID, Global.STAGE stage) {
		...
		for ( int i=0 ; i<Global.numSDBDomain ; i++ )
    		workers.push(new CollectCommittedTaskRunnable(jobID, i, stage)); //to query a domain for committedMap --d3
	 ...
	 * ideally, each task writes one row, but in reality, 
	 * each task writes several row where each row has less than 256 attributes because of SQS limitation
	 * 
